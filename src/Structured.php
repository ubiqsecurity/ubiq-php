...
class Structured {
    ...
    {
        ubiq_debug($creds, 'Starting deconstructFromPartialRules for ' . $string);
        ...
        foreach ($dataset->structured_config['passthrough_rules'] as $action) {
            if ($action['type'] === Structured::ENCRYPTION_RULE_TYPE_PASSTHROUGH) {
                ...
                ubiq_debug($creds, 'Parsing for partial encryption remove passthrough chars to result in ' . $string);
                ...
                ubiq_debug($creds, 'Parsing for partial encryption prefix ' . $prefix_str . ' and remainder ' . $string);
                ...
                ubiq_debug($creds, 'Parsing for partial encryption suffix ' . $suffix_str . ' and remainder ' . $string);
            }
        }
        ...
        ubiq_debug($creds, 'Finished deconstructFromPartialRules to ' . $string);
    ...
    {
        ubiq_debug($creds, 'Starting reconstructFromPartialRules for ' . $string);
        ...
        foreach ($rules as $action) {
            if ($action['type'] === Structured::ENCRYPTION_RULE_TYPE_PASSTHROUGH) {
                ...
                ubiq_debug($creds, 'Reconstructing for partial encryption add passthrough chars to result in ' . $string);
            } elseif ($action['type'] === Structured::ENCRYPTION_RULE_TYPE_PREFIX) {
                $string = $deconstruct_parts['prefix'] . $string;

                ubiq_debug($creds, 'Reconstructing for partial encryption prefix ' . $deconstruct_parts['prefix'] . ' to ' . $string);
            } elseif ($action['type'] === Structured::ENCRYPTION_RULE_TYPE_SUFFIX) {
                $string = $string . $deconstruct_parts['suffix'];

                ubiq_debug($creds, 'Reconstructing for partial encryption suffix ' . $deconstruct_parts['suffix'] . ' to ' . $string);
            }
        }
        ...
        if (!$passthrough_processed && !empty($passthrough_chars)) {
            ...
            ubiq_debug($creds, 'Parsing for partial encryption add passthrough chars to result in ' . $string);
        }

        ubiq_debug($creds, 'Finished reconstructFromPartialRules to ' . $string);
